...
//
// SPDX-License-Identifier: LGPL-3.0-only OR GPL-2.0-only OR GPL-3.0-only OR LicenseRef-KFQF-Accepted-GPL-2.0
...
{
    if (xml.attributes().hasAttribute(QString::fromUtf8(name)))
        return xml.attributes().value(QString::fromUtf8(name)).toUtf8();
    return QByteArray();
}
...
{
    QFile file(QString::fromUtf8(m_protocolFilePath));
    ...
    for (const auto &b : qAsConst(m_includes))
        printf("#include %s\n", b.constData());
